    ...
    SelectStmt *select_stmt, unique_ptr<LogicalNode> &logical_node)
{
  ...
  for (auto *table : tables) {
    ...
    table_get_nodes.push_back(std::unique_ptr<LogicalNode>(
  ...
  if(table_get_nodes.size() == 1) {
    root = std::move(table_get_nodes[0]);
        ...
        auto root_join = new JoinLogicalNode();
        for (auto &join_filter_stmt : select_stmt->join_filter_stmts()) {
          bool has_left = false;
          bool has_right = false;
          string table_name_left = dynamic_cast<FieldExpr *>(join_filter_stmt->filter_units()[0]->left_expr())->table_name();
          string table_name_right = dynamic_cast<FieldExpr *>(join_filter_stmt->filter_units()[0]->right_expr())->table_name();
          auto *join_logical_node = new JoinLogicalNode();
          join_logical_node->set_condition(_transfer_filter_stmt_to_expr(join_filter_stmt));

          for(auto &table_get_node : table_get_nodes) {
            if(table_get_node == nullptr)
            {
              continue;
            }
            auto *table_get_logical_node = dynamic_cast<TableGetLogicalNode *>(table_get_node.get());
            const char *table_name = const_cast<char *>(table_get_logical_node->table()->name());


            if (strcmp(table_name, table_name_left.c_str()) == 0)
            {
              has_left = true;
              join_logical_node->add_child(std::move(table_get_node));
              join_logical_node->table_names.push_back(const_cast<char *>(table_name));
            }
            if (strcmp(table_name, table_name_right.c_str()) == 0)
            {
              has_right = true;
              join_logical_node->add_child(std::move(table_get_node));
              join_logical_node->table_names.push_back(const_cast<char *>(table_name));
          ...
          {
            root = std::unique_ptr<LogicalNode>(join_logical_node);
            root_join = join_logical_node;
          }
          if(!has_left)
          {
            vector<char*> root_join_table_names = root_join->table_names;
            for (auto table_name : root_join_table_names)
            {
              if(table_name == table_name_left)
              {
                has_left = true;
                for(auto name : root_join_table_names)
                {
                  join_logical_node->table_names.push_back(name);
                }
                join_logical_node->add_child(std::move(root));
                join_logical_node->swap_children();
                root = std::unique_ptr<LogicalNode>(join_logical_node);
          ...
          if(!has_right)
          {
            ...
            for (auto table_name : root_join_table_names)
            {
              if(table_name == table_name_right)
              {
                has_right = true;
                for(auto name : root_join_table_names)
                {
        ...
        //root = std::move(table_get_nodes_list.front());
        }
