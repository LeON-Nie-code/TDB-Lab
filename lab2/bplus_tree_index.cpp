...
RC BplusTreeIndex::insert_entry(const char *record, const RID *rid)
{
  ...
  // 从记录中提取多字段索引的字段值作为键
  // 这里需要根据索引元数据和字段元数据，确定如何从记录中提取出多字段索引的字段值
  int len = 0;
  for(int i = 0; i < multi_field_metas_.size(); i++) {
        len += multi_field_metas_[i].len();
  }
  const char **key = new const char *[multi_field_metas_.size()];

  for(int i = 0; i < multi_field_metas_.size(); i++) {
    char *temp = new char[multi_field_metas_[i].len()];
    std::memcpy(temp, record + multi_field_metas_[i].offset(), multi_field_metas_[i].len());
    key[i] = temp;
  }

  // 调用BplusTreeHandler的insert_entry完成插入操作
  RC rc = index_handler_.insert_entry(key, rid, multi_field_metas_.size());
  if (rc == RC::RECORD_DUPLICATE_KEY && index_meta_.is_unique()) {
    // 如果是唯一索引并且存在重复的字段值，则返回记录重复键错误
    LOG_WARN("Duplicate key found for unique index.");
    return RC::RECORD_DUPLICATE_KEY;
  } else if (rc != RC::SUCCESS) {
    // 如果插入操作失败，则返回相应错误码
    LOG_WARN("Failed to insert entry into B+ tree.");
    return rc;
  }
...
RC BplusTreeIndex::delete_entry(const char *record, const RID *rid)
{
  ...
  // 调用BplusTreeHandler的delete_entry完成删除操作
  RC rc = index_handler_.delete_entry(key, rid);
  if (rc != RC::SUCCESS) {
    // 如果删除操作失败，则返回相应错误码
    LOG_WARN("Failed to delete entry from B+ tree.");
    return rc;
  }
  return RC::SUCCESS;
}
