...
RC Table::insert_record(Record &record)
{
  ...
  for (Index *index : indexes_) {
    ...
    if (rc != RC::SUCCESS) {
      // 处理异常情况，可能需要撤销插入操作
      LOG_ERROR("Failed to insert record into index. table=%s, index=%s, rc=%s", name(), index->index_meta().name(), strrc(rc));
      // 撤销插入操作
      RC rollback_rc = record_handler_->delete_record(&record.rid());
      if (rollback_rc != RC::SUCCESS) {
        LOG_ERROR("Failed to rollback insert operation. table name=%s, rc=%s", table_meta_.name(), strrc(rollback_rc));
      }
      return rc;
    }
  }
