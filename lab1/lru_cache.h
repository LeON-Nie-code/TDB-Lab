...
public:
  ...
  void pop(Value *&value)
  {
    ...
    value = &(lru_tail_->value_);
    remove(lru_tail_->key_);
  }

  Value* foreach_value(std::function<bool(const Value&)> func) {
    for (ListNode* node = lru_front_; node != nullptr; node = node->next_) {
      bool ret = func(node->value_);
      if (!ret) {
        return &(node->value_);
      }
    }
    return nullptr; // 或者返回 nullptr，表示没有找到符合条件的值
  }
