...
RC FileBufferPool::close_file()
{
  ...
  RC rc_hdr = flush_page(*hdr_frame_);
  if(rc_hdr != RC::SUCCESS) {
    LOG_ERROR("Failed to flush header page %s, due to %s.", file_name_.c_str(), strrc(rc_hdr));
  }
  hdr_frame_->unpin();
  ...
  LOG_INFO(file_header_->to_string().c_str());
...
//  5. 记录和返回成功
  ...
  if (writen(file_desc_, &page, BP_PAGE_SIZE) != 0) {
    LOG_ERROR("Failed to flush page %s, file_desc:%d, page num:%d, due to failed to write data:%s",
              file_name_.c_str(), file_desc_, frame.page_num(), strerror(errno));
    return RC::IOERR_WRITE;
  }
  LOG_INFO("Page %d flushed to disk", frame.page_num());
  LOG_INFO("page data: %s", page.data);
  // 清除frame的脏标记
  frame.clear_dirty();
  LOG_INFO(file_header_->to_string().c_str());
  return RC::SUCCESS;
...
{
  // 根据LRU算法选择需要被驱逐的页面
  Frame *evict_frame = frame_manager_.get(file_desc_, page_num);
  if (evict_frame == nullptr) {
    LOG_ERROR("No available frames to evict page %d", page_num);
    return RC::BUFFERPOOL_NOBUF;
  }

  // 如果被驱逐的页面为脏页，则需要先将其刷盘
  if (evict_frame->dirty()) {
    RC rc = flush_page(*evict_frame);
    if (rc != RC::SUCCESS) {
      LOG_ERROR("Failed to flush page during eviction");
      return rc;
    }
  }
  ...
  // 从缓冲池中移除被驱逐的页面
  evict_frame->reset();
  evict_frame->page_num();
  frame_manager_.remove_frame(file_desc_, page_num);

  LOG_INFO("Page %d evicted from buffer pool", evict_frame->page_num());
  return RC::SUCCESS;
}
/**
...
RC FileBufferPool::evict_all_pages()
{
        ...
        while (iter.has_next()) {
            ...
            if (frame == nullptr) {
                LOG_ERROR("Failed to evict all pages, no available frames");
                return RC::BUFFERPOOL_NOBUF;
            }
            if (frame->dirty()) {
            ...
            frame->page_num();
            frame_manager_.remove_frame(file_desc_, page_num);
            LOG_INFO("Page %d evicted from buffer pool", frame->page_num());
        }
...
{
  LOG_INFO("load page %s:%d", file_name_.c_str(), page_num);
  int64_t offset = ((int64_t)page_num) * BP_PAGE_SIZE;
  if (lseek(file_desc_, offset, SEEK_SET) == -1) {
    LOG_ERROR("Failed to load page %s:%d, due to failed to lseek: %s (errno=%d).", file_name_.c_str(), page_num, strerror(errno), errno);
    return RC::IOERR_SEEK;
  }
  ...
  if (ret != 0) {
    LOG_ERROR("Failed to load page %s, file_desc: %d, page num: %d, due to failed to read data: %s (errno=%d), ret=%d, page count=%d",
              file_name_.c_str(), file_desc_, page_num, strerror(errno), errno, ret, file_header_->allocated_pages);
...
{
        std::scoped_lock lock_guard(lock_);
        //find the file buffer pool
        auto iter = fd_buffer_pools_.find(frame.file_desc());
        if (iter == fd_buffer_pools_.end()) {
            LOG_ERROR("Failed to flush page, no available buffer pool");
            return RC::BUFFERPOOL_NOBUF;
        }
        FileBufferPool *bp = iter->second;
