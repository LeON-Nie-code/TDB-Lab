...
RC MvccTrx::insert_record(Table *table, Record &record)
{
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to insert record while inserting. rc=%s", strrc(rc));
    return rc;
    }
  log_manager_->append_record_log(LogEntryType::INSERT, trx_id_, table->table_id(), record.rid(), record.len(), 0, record.data());
...
RC MvccTrx::delete_record(Table *table, Record &record)
{
  ...
  Field begin_xid_field, end_xid_field;
  trx_fields(table, begin_xid_field, end_xid_field);
  if (visit_record(table, record, false) != RC::SUCCESS) {
    return RC::RECORD_INVISIBLE;
  }
  end_xid_field.set_int(record, -trx_id_);

  log_manager_->append_record_log(LogEntryType::DELETE, trx_id_, table->table_id(), record.rid(), record.len(), 0, record.data());
...
RC MvccTrx::start_if_need()
{
  ...
  log_manager_->append_begin_trx_log(trx_id_);
...
RC MvccTrx::rollback()
{
  ...
  log_manager_->append_rollback_trx_log(trx_id_);//?
...
RC MvccTrx::redo(Db *db, const LogEntry &log_entry)
{
  ...
  switch (log_entry.log_type()) {
    case LogEntryType::INSERT: {
      LOG_INFO("redo insert record. log entry=%s", log_entry.to_string().c_str());
      Table *table = nullptr;
      const RecordEntry &record_entry = log_entry.record_entry();
      Record record;
      table = db->find_table(record_entry.table_id_);
      if (table == nullptr) {
        LOG_WARN("failed to find table while redoing. table id=%d", record_entry.table_id_);
        return RC::MY_ERROR;
      }

      record.set_data(record_entry.data_, record_entry.data_len_);
      record.set_rid(record_entry.rid_);
      LOG_INFO("redo insert record. record=%s", record.data());


      if(table->recover_insert_record(record) != RC::SUCCESS){
        LOG_WARN("failed to recover insert record while redoing. rid=%s", record_entry.rid_.to_string().c_str());
        return RC::MY_ERROR;

      }

      Record new_record;
      table->get_record(record_entry.rid_, new_record);
      LOG_INFO("get record from table =%s", new_record.data());
//      RowTuple tuple = RowTuple();
//        tuple._set_record(&new_record);
////        LOG_INFO("redo insert record. record=%s", tuple.to_string().c_str());
    ...
    case LogEntryType::DELETE: {
      LOG_INFO("redo delete record. log entry=%s", log_entry.to_string().c_str());
      Table *table = nullptr;
      const RecordEntry &record_entry = log_entry.record_entry();
      Record record;
      table = db->find_table(record_entry.table_id_);
      ...
      RC rc = table->get_record(record_entry.rid_, record);
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to get record while redoing. rid=%s, rc=%s", record_entry.rid_.to_string().c_str(), strrc(rc));
        return rc;
      }
      ...
      table->visit_record(record_entry.rid_, false, [this, &end_xid_field](Record &record) {
        end_xid_field.set_int(record, trx_kit_.max_trx_id());
      });
    ...
    case LogEntryType::MTR_COMMIT: {
        LOG_INFO("redo commit trx. log entry=%s", log_entry.to_string().c_str());

      // TODO [Lab5] 需要同学们补充代码，相关提示见文档
      int32_t commit_xid = log_entry.commit_entry().commit_xid_;
      if(commit_with_trx_id(commit_xid) != RC::SUCCESS){
        LOG_WARN("failed to commit trx while redoing. trx id=%d", log_entry.trx_id());
        return RC::MY_ERROR;
      }
    ...
    case LogEntryType::MTR_ROLLBACK: {
        LOG_INFO("redo rollback trx. log entry=%s", log_entry.to_string().c_str());
